...
parser = argparse.ArgumentParser(
    description="This program will download bacterial sequencing data from NCBI's FTP server")
...
parser.add_argument("-t", "--type", default="fasta", help="file type",
                    choices=["genbank", "fasta", "gff", "feature_table"], type=str)
...
try:
    subprocess.call(
        "rsync -Lrtv %s rsync://ftp.ncbi.nlm.nih.gov/genomes/%s/bacteria/%s/latest_assembly_versions/*/ %s" % (
        file_type, NCBI_database, args.bacterium, "."), shell=True)
except:
